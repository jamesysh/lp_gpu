...
//Pellet density integral calculation
void Octree::computeIntegral(const double* mass, double* left_integral, double* right_integral, int fluidnum){
	uint64_t mask=computeKey(m_dBoundingBox_min_x, m_dBoundingBox_max_y, m_dBoundingBox_max_z);
	std::unordered_map<uint64_t, std::pair<double,double>> hash;
	int j=0;
	uint64_t yzkey;
	double yzsize=(m_dBoundingBox_max_y-m_dBoundingBox_min_y)*(m_dBoundingBox_max_z-m_dBoundingBox_min_z);
	auto ite = hash.begin();
	for(int i=0;i<m_iMaxDepth;i++)
		yzsize=0.25*yzsize;
	for(int i=0;i<=m_iTotalNumberOfParticles;i++){
		if(i==0 || i==m_iTotalNumberOfParticles || m_vParticleKeyIndex[i].key!=m_vParticleKeyIndex[i-1].key){
			for(;j<i;j++)
			{
                                left_integral[m_vParticleKeyIndex[j].index]=ite->second.first+0.5*ite->second.second;
			}
			if(i==m_iTotalNumberOfParticles)
				break;
			yzkey=(m_vParticleKeyIndex[i].key&mask);
			ite = hash.find(yzkey);
			if(ite==hash.end())
			{
				hash.insert({yzkey,{0,0}});
				ite = hash.find(yzkey);
			}
			ite->second.first+=ite->second.second;
			ite->second.second=0;
}
//		if(m_vParticleKeyIndex[i].index<fluidnum)
			ite->second.second+=mass[m_vParticleKeyIndex[i].index]/yzsize;
	}
	for(int i=0;i<m_iTotalNumberOfParticles;i++){
		if(yzkey!=m_vParticleKeyIndex[i].key&mask)
		{
                	yzkey=(m_vParticleKeyIndex[i].key&mask);
			ite = hash.find(yzkey);
		}
                right_integral[m_vParticleKeyIndex[i].index]=ite->second.first+ite->second.second-left_integral[m_vParticleKeyIndex[i].index];
	}

}

//Pellet density integral calculation using AP-Cloud computational nodes
void Octree::computeIntegralAPCloud(const double* mass, double* left_integral, double* right_integral, int fluidnum, int maxnp){
	std::vector<int> child,old_index,left_neigh,right_neigh,level,phint(8,0);
	std::vector<double> x_size(m_iMaxDepth+1),y_size(m_iMaxDepth+1),z_size(m_iMaxDepth+1),local_sum,left_sum,right_sum,left_area,right_area;
	std::queue<int> left_queue,right_queue;

//cout<<"Build APCloud computational nodes"<<endl;
//build up apcloud computational nodes
//start from root node
	child.push_back(-1);
	old_index.push_back(0);	
	left_neigh.push_back(-1);
	...
	int deepest=0;
	while(i<iend){
                int inode=old_index[i];
		//check if the current node is fine enough
		if(inode<0 || level[i]==m_iMaxDepth || m_vNumberOfContainedParticles[inode]<=maxnp)
		{
			i++;
			if(inode>=0 && level[i]==m_iMaxDepth && m_vNumberOfContainedParticles[inode]>maxnp)
				flag=true;
		...
		//open ith cell and add all its children
		child[i]=iend;
		//resize vectors usign placeholder
		child.insert(child.end(),phint.begin(),phint.end());
		old_index.insert(old_index.end(),phint.begin(),phint.end());
		left_neigh.insert(left_neigh.end(),phint.begin(),phint.end());
		right_neigh.insert(right_neigh.end(),phint.begin(),phint.end());
		level.insert(level.end(),phint.begin(),phint.end());
		//initialize the states
		for(int j=iend;j<iend+8;j++)
		{
			old_index[j]=-1;
			level[j]=level[i]+1;
			if(deepest<level[j])
		...
		//set neighbors
		for(int j=iend;j<iend+4;j++)
		{
			if(left_neigh[i]>=0 && child[left_neigh[i]]>=0)
			{
				left_neigh[j]=child[left_neigh[i]]+4+j-iend;
				right_neigh[child[left_neigh[i]]+4+j-iend]=j;
			}
			else
				left_neigh[j]=left_neigh[i];
			right_neigh[j]=j+4;
		}
                for(int j=iend+4;j<iend+8;j++)
                {
                        if(right_neigh[i]>=0 && child[right_neigh[i]]>=0)
                        {
                                right_neigh[j]=child[right_neigh[i]]+j-iend-4;
                                left_neigh[child[right_neigh[i]]+j-iend-4]=j;
                        }
                        else
                                right_neigh[j]=right_neigh[i];
			left_neigh[j]=j-4;
                }
		//look for old index from octree
		for(int j=m_vFirstChildIndex[inode];j<m_vFirstChildIndex[inode]+m_vNumberOfChildren[inode];j++)
		{
			double tx=(m_vUpperLimitOfX[j]+m_vLowerLimitOfX[j])-(m_vUpperLimitOfX[inode]+m_vLowerLimitOfX[inode]);
                        double ty=(m_vUpperLimitOfY[j]+m_vLowerLimitOfY[j])-(m_vUpperLimitOfY[inode]+m_vLowerLimitOfY[inode]);
                        double tz=(m_vUpperLimitOfZ[j]+m_vLowerLimitOfZ[j])-(m_vUpperLimitOfZ[inode]+m_vLowerLimitOfZ[inode]);
			int inc=0;
			if(tx>0) inc+=4;
			if(ty>0) inc+=2;
			if(tz>0) inc+=1;
			old_index[iend+inc]=j;
		}
		i++;
		iend+=8;
	}
	...
	cout<<"Generated "<<n<<" APCloud computational nodes, max level = "<<deepest<<endl;
	if(flag==true)
		cout<<"Warning: Octree is not deep enough. Increase octree depth might improve density integral estimation."<<endl;

	x_size[0]=m_vUpperLimitOfX[0]-m_vLowerLimitOfX[0];
        y_size[0]=m_vUpperLimitOfY[0]-m_vLowerLimitOfY[0];
        z_size[0]=m_vUpperLimitOfZ[0]-m_vLowerLimitOfZ[0];
	for(int j=1;j<=m_iMaxDepth;j++){
		x_size[j]=x_size[j-1]/2;
		y_size[j]=y_size[j-1]/2;
		z_size[j]=z_size[j-1]/2;
	}

	double tol=y_size[m_iMaxDepth]*z_size[m_iMaxDepth]/4;

	local_sum.resize(n,0);
	left_sum.resize(n,0);
	right_sum.resize(n,0);
	left_area.resize(n,0);
	right_area.resize(n,0);
//cout<<"Initialize area and calculate local sum"<<endl;
//initialize area and calculate local_sum
	for(int i=0;i<n;i++)
	{
		int l=level[i];
		if(child[i]==-1)
		{
			...
			int inode=old_index[i];
			if(inode>=0)
				for(int k=m_vFirstParticleIndex[inode];k<m_vFirstParticleIndex[inode]+m_vNumberOfContainedParticles[inode];k++)
				{
					int j=m_vParticleKeyIndex[k].index;
					if(j<fluidnum)
						sum+=mass[j];
				}
			sum=sum/y_size[l]/z_size[l];
			local_sum[i]=sum;
		}
		left_area[i]=right_area[i]=y_size[l]*z_size[l];
	}
//cout<<"Calcualte left density integral"<<endl;
	//calculate left density integral
	while(!left_queue.empty())
	{
		int i=left_queue.front();
		left_queue.pop();
		...
		if(inode>=0)
			...
			{
				int j=m_vParticleKeyIndex[k].index;
				left_integral[j]=left_sum[i]+local_sum[i]*0.5;
			}
		std::queue<int> nextqueue;
		if(right_neigh[i]>=0)
			nextqueue.push(right_neigh[i]);
		while(!nextqueue.empty()){
			int j=nextqueue.front();
			nextqueue.pop();
			if(child[j]>=0)
			{
				nextqueue.push(child[j]);
                                nextqueue.push(child[j]+1);
                                nextqueue.push(child[j]+2);
                                nextqueue.push(child[j]+3);
			}
			else
			{
				left_area[j]-=y_size[l]*z_size[l];
				int lj=level[j];
				double contribution=left_sum[i]+local_sum[i];
				if(lj<l)
					contribution*=y_size[l]*z_size[l]/y_size[lj]/z_size[lj];
				left_sum[j]+=contribution;
				if(left_area[j]<=tol)
					left_queue.push(j);
			}
		}
...
//cout<<"Calcualte right density integral"<<endl;
	//calculate right density integral
        while(!right_queue.empty())
        {
                int i=right_queue.front();
                right_queue.pop();
                int inode=old_index[i],l=level[i];
                if(inode>=0)
                        for(int k=m_vFirstParticleIndex[inode];k<m_vFirstParticleIndex[inode]+m_vNumberOfContainedParticles[inode];k++)
                        {
                                int j=m_vParticleKeyIndex[k].index;
                                right_integral[j]=right_sum[i]+local_sum[i]*0.5;
                        }
                std::queue<int> nextqueue;
                if(left_neigh[i]>=0)
                        nextqueue.push(left_neigh[i]);
                while(!nextqueue.empty()){
                        int j=nextqueue.front();
                        nextqueue.pop();
                        if(child[j]>=0)
                        {
                                nextqueue.push(child[j]+4);
                                nextqueue.push(child[j]+5);
                                nextqueue.push(child[j]+6);
                                nextqueue.push(child[j]+7);
                        }
                        else
                        {
                                right_area[j]-=y_size[l]*z_size[l];
                                int lj=level[j];
                                double contribution=right_sum[i]+local_sum[i];
                                if(lj<l)
                                        contribution*=y_size[l]*z_size[l]/y_size[lj]/z_size[lj];
                                right_sum[j]+=contribution;
                                if(right_area[j]<=tol)
                                        right_queue.push(j);
                        }
                }
...
//cout<<"Integral calcualted"<<endl;
}


inline uint64_t Octree::computeKey(const double& x, const double& y, const double& z)
{
