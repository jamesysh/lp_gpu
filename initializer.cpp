...
// this function is not supported and should not be called in the current version!	
...
//	}
	...
	save<<"m_iUseLimiter "<<m_iUseLimiter<<endl;
        ...
        iss.str(lines[i++]);
        iss>>m_iDensityEstimatorType;
        save<<"m_iDensityEstimatorType "<<m_iDensityEstimatorType<<endl;
	...
	iss.str(lines[i++]);
	iss>>m_iFixParticles;
	save<<"m_iFixParticles "<<m_iFixParticles<<endl;
	iss.str(lines[i++]);
	iss>>m_iOutputError;
	save<<"m_iOutputError "<<m_iOutputError<<endl;
	iss.str(lines[i++]);
	iss>>m_sExactSolutionName;
	save<<"m_sExactSolutionName "<<m_sExactSolutionName<<endl;
        iss.str(lines[i++]);
        iss>>m_sAuxiliaryOutput;
        save<<"m_sAuxiliaryOutput "<<m_sAuxiliaryOutput<<endl;
	iss.str(lines[i++]);
...
//			m_iMaxNeighbourNumInOneDir=8*m_iNumRow1stOrder; 
	...
	m_iTreeDepth = 7;//optimal performance for most problems
	if(m_iPelletDistribution>0)
		m_iTreeDepth = 20;//deep octree is needed for density integral calculation in pellet ablation simulation
//        if(m_iDimension==3) m_iTreeDepth = 4;//5
...
//		m_iBoundaryNum = 0;	
		...
		if(m_iIfHex==true) 
			...
			m_iFluidNum = initGeometryAndStateOnRandomDistribution(saveData);
//		m_iFluidNum = initGeometryAndStateOnHexPackingTemp(saveData);//for 2d shock tube
		...
		if(m_iIfHex==true)
			...
			initGeometryAndStateOnRandomDistribution(saveData);
...
size_t Initializer::initGeometryAndStateOnHexPackingTemp(bool saveData) {
	...
	if(m_iDimension==2) {
		...
		for(size_t p=0; p<objs.size(); p++) {
			...
			hex2D.getInitialPerturbation(m_fInitialPerturbation);
			...
			hex2DD.getInitialPerturbation(m_fInitialPerturbation);
...
size_t Initializer::initGeometryAndStateOnRandomDistribution(bool saveData){

        const vector<Geometry*>& objs = m_vFluidObj;
        const vector<State*>& states = m_vFluidObjState;

	size_t ParticlePerObject=9070;
        srand(1);

	if(saveData==false)
		return ParticlePerObject*objs.size();

	for(size_t p=0;p<objs.size();p++){
		for(int numParticle=0;numParticle<ParticlePerObject;numParticle++){
			double x,y,z;
			objs[p]->randomlocation(x,y,z);
                        m_vPositionX[numParticle] = x;
                        m_vPositionY[numParticle] = y;
			if(m_iDimension==3)
                        	m_vPositionZ[numParticle] = z;
			if(!objs[p]->operator()(x,y,z)) {numParticle--;continue;}
                        m_vVolume[numParticle] = 1./states[p]->density(x,y,z);
			m_vMass[numParticle] = 0.1*0.1*sqrt(3)*0.5/m_vVolume[numParticle];
//			m_vMass[numParticle] = 76.79/ParticlePerObject;
//                        m_vMass[numParticle] = states[p]->gettotalmass()/ParticlePerObject;
                        m_vPressure[numParticle] = states[p]->pressure(x,y,z);
			if(m_iDimension==3)
                        	states[p]->velocity(x,y,z,m_vVelocityU[numParticle],m_vVelocityV[numParticle],m_vVelocityW[numParticle]);
			if(m_iDimension==2)
			{
				double ztemp=0;
                                states[p]->velocity(x,y,z,m_vVelocityU[numParticle],m_vVelocityV[numParticle],ztemp);
			}
                        m_vSoundSpeed[numParticle] = m_pEOS->getSoundSpeed(m_vPressure[numParticle],1./m_vVolume[numParticle]);
		}	
...
//							else
								m_vPositionX[numParticle] = x;
//	                                                if(x>0) m_vPositionX[numParticle]=x-0.6*h_r;
							m_vPositionY[numParticle] = y;
...
//                                                        if(x>0) m_vPositionX[numParticle]=x-0.6*h_r;
		...
		for(size_t p=0; p<objs.size(); p++) {
			...
			hex3D.getInitialPerturbation(m_fInitialPerturbation);
			//get parameters of hexagonal packing
			size_t l0,l1;
...
// if(x>0) m_vPositionX[numParticle]=x-0.6*h_r;									
									...
									m_vPressure[numParticle] = states[p]->pressure(x,y,z);
                                                        		m_vMass[numParticle] = sqrt(2)*4*h_r*h_r*h_r/m_vVolume[numParticle];
//m_vMass[numParticle] = 8*h_r*h_r*h_r/m_vVolume[numParticle];
									states[p]->velocity(x,y,z,
										m_vVelocityU[numParticle],m_vVelocityV[numParticle],m_vVelocityW[numParticle]);
									m_vSoundSpeed[numParticle] = 
										m_pEOS->getSoundSpeed(m_vPressure[numParticle],1./m_vVolume[numParticle]);
...
void Initializer::initPellet(){
	...
	m_iMaxParticlePerCell = 50;//maximum number of particles per APCloud node in density integral calculation
	...
	if(m_iPelletDistribution==1)
	{
		...
		m_vPelletRadius[0]=0.2;
		m_vPelletInnerRadius[0]=0.2;
		m_vPelletPositionX[1]=2;
		m_vPelletPositionY[1]=2;
		m_vPelletPositionZ[1]=2;
