...
struct KeyIndex
{
  uint64_t key;//Should be a 32/64 bit integer. With a 32 bit integer we can use Octrees of depth 10. With a 64 bit integer we can use octrees of depth 20.
  int index;
...
private:
   ...
   */
  inline uint64_t computeKey(const double& x, const double& y, const double& z);
