...
//----------------------------GPU ARRAY MEMORY ALLOCATION----------------------------------------
        ...
        cudaMalloc((void**)&d_m_vSoundSpeed,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1Pressure,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1Volume,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1SoundSpeed,sizeof(double)*m_iCapacity);
        ...
        cudaMalloc((void**)&d_m_vLPFOrderSouth,sizeof(int)*m_iCapacity);
        cudaMalloc((void**)&d_m_vVelocityU,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1VelocityU,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp2VelocityU,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vVelocityV,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1VelocityV,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp2VelocityV,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vPositionX,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vPositionY,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vNeighbourListRight,sizeof(int)*m_iCapacity*m_iMaxNeighbourNumInOneDir);
    ...
    if(m_iNumberofPellet){
        cudaMalloc((void**)&d_deltaq,sizeof(double)*m_iCapacity);
    }
        if(m_iDimension == 3){
        
        cudaMalloc((void**)&d_m_vNeighbourListUp,sizeof(int)*m_iCapacity*m_iMaxNeighbourNumInOneDir);
        cudaMalloc((void**)&d_m_vNeighbourListDown,sizeof(int)*m_iCapacity*m_iMaxNeighbourNumInOneDir);       
        cudaMalloc((void**)&d_m_vNeighbourListUpSize,sizeof(int)*m_iCapacity);
        cudaMalloc((void**)&d_m_vNeighbourListDownSize,sizeof(int)*m_iCapacity);
        cudaMalloc((void**)&d_m_vPositionZ,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vVelocityW,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp1VelocityW,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vTemp2VelocityW,sizeof(double)*m_iCapacity);
        cudaMalloc((void**)&d_m_vLPFOrderUp,sizeof(int)*m_iCapacity);
        cudaMalloc((void**)&d_m_vLPFOrderDown,sizeof(int)*m_iCapacity);
        }
...
ParticleData::~ParticleData() {
    ...
    cudaFree(d_m_vNeighbourListRightSize);
    cudaFree(d_m_vNeighbourListLeftSize);
    cudaFree(d_m_vNeighbourListNorthSize);
    cudaFree(d_m_vNeighbourListSouthSize);
    cudaFree(d_m_vPressure);
    cudaFree(d_m_vVolume);
    cudaFree(d_m_vSoundSpeed);
    cudaFree(d_m_vTemp1Pressure);
    cudaFree(d_m_vTemp1Volume);
    cudaFree(d_m_vTemp1SoundSpeed);
    cudaFree(d_m_vTemp2Pressure);
    cudaFree(d_m_vTemp2Volume);
    cudaFree(d_m_vTemp2SoundSpeed);
    cudaFree(d_m_vLPFOrderRight);
    cudaFree(d_m_vLPFOrderLeft);
    cudaFree(d_m_vLPFOrderNorth);
    cudaFree(d_m_vLPFOrderSouth);
    cudaFree(d_m_vVelocityU);
    cudaFree(d_m_vTemp1VelocityU);
    cudaFree(d_m_vTemp2VelocityU);
    cudaFree(d_m_vVelocityV);
    cudaFree(d_m_vTemp1VelocityV);
    cudaFree(d_m_vTemp2VelocityV);
    cudaFree(d_m_vPositionX);
    cudaFree(d_m_vPositionY);
    cudaFree(d_m_vNeighbourListRight);
    ...
    if(m_iDimension == 3){
        ...
        cudaFree(d_m_vNeighbourListDown);
        cudaFree(d_m_vNeighbourListUpSize);
        cudaFree(d_m_vNeighbourListDownSize);
        cudaFree(d_m_vPositionZ);
        cudaFree(d_m_vVelocityW);
        cudaFree(d_m_vTemp1VelocityW);
        cudaFree(d_m_vTemp2VelocityW);
        cudaFree(d_m_vLPFOrderUp);
        cudaFree(d_m_vLPFOrderDown);
    }
